...
#
# Usage:
#   curl -sL URL/export-perplexity.sh -o /tmp/export-perplexity.sh && bash /tmp/export-perplexity.sh
# ─────────────────────────────────────────────────────────────────────
...
if [[ "$RUNTIME" == "node" ]]; then
  ...
  curl -sL "$GIST_BASE/export-perplexity.mjs" -o "$SCRIPT"
  ...
  echo ""
  curl -sL "$GIST_BASE/export-perplexity.py" -o "$SCRIPT"
  [[ -s "$SCRIPT" ]] || { echo "Error: Failed to download script."; exit 1; }
  python3 "$SCRIPT"
fi
